...
const props = defineProps(['octave', 'full']);
var svgRef: Ref = ref(null);

function getDisplayedNotes(): Array<string> {
  if (props.full) {
    return notes;
  } else {
    return ["C"]
...
<template>
  <div>
    ...
    <span v-if="props.full">
      <svg :ref="(el) => { svgRef = el || svgRef }" width="400" height="227" viewBox="0 0 400 227" fill="none" xmlns="http://www.w3.org/2000/svg">
        <g id="Keyboard">
          <g id="c">
    ...
    <span v-if="!props.full">
      <svg :ref="(el) => { svgRef = el || svgRef }" width="90" height="227" viewBox="0 0 90 227" fill="none" xmlns="http://www.w3.org/2000/svg">
        <g id="Keyboard">
          <g id="c">
          <path id="C" fill-rule="evenodd" clip-rule="evenodd" d="M16.4575 0C7.36827 0 0 7.36827 0 16.4575V210.263C0 219.352 7.36828 226.721 16.4575 226.721H36.9838C46.073 226.721 53.4413 219.352 53.4413 210.263V151.041C52.8235 151.111 52.1954 151.147 51.5587 151.147H48.8462C39.757 151.147 32.3887 143.779 32.3887 134.69V0H16.4575Z" fill="white"/>
          </g>
